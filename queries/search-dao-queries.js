...
var getByPartialName = function(callback){
    ...
    var _query = "" +
        "SELECT osm_id, name, lod FROM "+table+" NATURAL JOIN "+styleTable+" WHERE is_deleted = false AND lod <= $1::integer AND osm_id IN (SELECT DISTINCT osm_id FROM "+nameRelTable+" WHERE name_id IN (SELECT name_id FROM "+nameTable+" WHERE name ILIKE $2::varchar)) ";
    return _query;
}
