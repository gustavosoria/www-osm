...
var getPointByOsmId = function(){
    var _query = "SELECT array_to_json(array_agg(planet_osm_point)) as way, ST_AsGeoJSON(way) as geom FROM planet_osm_point WHERE osm_id = $1::bigint AND is_deleted = false GROUP BY way";
    return _query;
}
...
var getPointsByBBox = function(){
    ...
    var _query = "SELECT *, ST_AsGeoJSON(geomm) as geom " +
        "FROM (	SELECT * " +
        "FROM (	SELECT osm_id, ST_Intersection(planet_osm_point.way, bbox.geom) as geomm " +
        "FROM planet_osm_point, (SELECT ST_SetSRID(ST_MakeBox2d(ST_MakePoint($1::float, $2::float), ST_MakePoint($3::float, $4::float)), 4326) as geom) as bbox " +
        "WHERE ST_Intersects(planet_osm_point.way, bbox.geom)) AND is_deleted = false " +
        "fc ) as t";
